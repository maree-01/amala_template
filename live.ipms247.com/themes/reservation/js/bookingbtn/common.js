...
&& rzurldata[2]!=258 && rzurldata[2]!=177)
		...
		//console.log(rzurldata[2]);
             //   return false;
                     ...
                     //Chandrakant - 2.0.10 - 27 August 2018 - START
                     //Purpose : CNAMEiOSIssue - Fix http base BE link block by safari when it runs from https base website.
                     //var http_var='https://';
                     //if (rzurldata2[2]!='live.ipms247.com') {
                     //   http_var='http://';
                     //}
                     http_var = rzhttp[0]+'//';
                     //Chandrakant - 2.0.10 - 27 August 2018 - END
                     
                     var rz_frm_action=http_var+rzurldata2[2]+'/booking/book-rooms-'+rzurldata[2];
                     jQuery.ajax({
                         type: "POST",
                         url: http_var+rzurldata2[2]+'/booking/service.php',
                         async:false,
                         data: param, 
                         success: function(response) {
                           
                              if (response.trim()!='2')
                                 rz_frm_action=http_var+rzurldata2[2]+'/booking/mroominfo.php?HotelId='+rzurldata[2]+'&eZ_chkin='+jQuery("#eZ_chkin").val()+'&eZ_Nights='+jQuery("#eZ_Nights").val()+'&eZ_adult='+jQuery("#eZ_adult").val()+'&eZ_child='+jQuery("#eZ_child").val()+'&eZ_room='+jQuery("#eZ_room").val()+'&promotion='+promo+'&ArrvalDt='+jQuery("#ArDt").val()+'&executepage=mroominfo';
                        },
                        complete:function(){
                           // document._resBBBox.action=rz_frm_action;
                           // document._resBBBox.submit();
                           //Prashant Patel  - 10/03/2022 - Purpose : comment above code and added below code for mobile layout booking engine - [ RES-2985] --START
                           document.forms['_resBBBox'].action=rz_frm_action;
                           document.forms['_resBBBox'].submit();
                           //Prashant Patel  - 10/03/2022 - [ RES-2985] --END
                        }
                     });
...
//Chinmay Gandhi - 28th June 2019 - Start
//Purpose : Develop dynamic booking box for prime layout
function BB_valiDateFomData_prime()
{
   ...
   var calFormat = (jQuery("input[id$='calformat']").val());
   var startDate_format=convertdate_format(jQuery("#checkIn").val(),calFormat);
   var endDate_format=convertdate_format(jQuery("#checkOut").val(),calFormat);
...
//Chinmay Gandhi - 28th June 2019 - End
